...
import { PROP_IMAGES } from '../actions/types';
...
export default class PropImages extends BaseDataInjector {
  constructor(widgetListener, ref) {
    super();
    this.ref = ref;
    this.refId = ref.id;
    this.config = {
      title: 'Upload prop images',
      supportedFormat: 'image/jpeg, image/png',
      pickerType: PROP_IMAGES,
  ...
  getDisplayMessage = () =>
    `You can select ${MAX_SELECT_ALLOWED} photo for props`;
...
function propPatch(refId, config, successListener, failureListener) {
  new AxiosBuilder(`/props/${refId}/`, config)
    .PATCH()
    .then(result => {
